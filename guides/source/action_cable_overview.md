...
incorporate real-time features into your Quails application.
...
Quails application. It allows for real-time features to be written in Ruby in the
...
regular MVC setup. By default, Quails creates a parent `ApplicationCable::Channel` class
...
established using the following JavaScript, which is generated by default by Quails:
...
Broadcasts are called elsewhere in your Quails application:
...
See the [quails/actioncable-examples](https://github.com/quails/actioncable-examples)
repository for a full example of how to setup Action Cable in a Quails app and adding channels.
...
The file must specify an adapter for each Quails environment. See the
...
This may change in the future. [#27214](https://github.com/quails/quails/issues/27214)
...
```ruby
config.action_cable.allowed_request_origins = ['http://rubyonquails.com', %r{http://ruby.*}]
```
...
Action Cable can run alongside your Quails application. For example, to
...
in new Quails applications is the asynchronous (`async`) adapter.
...
Quails models with no problem, as long as you haven't committed any thread-safety sins.
