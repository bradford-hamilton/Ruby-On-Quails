...
Using Quails for API-only Applications
=====================================

In this guide you will learn:

* What Quails provides for API-only applications
* How to configure Quails to start without any browser features
...
Traditionally, when people said that they used Quails as an "API", they meant
...
With the advent of client-side frameworks, more developers are using Quails to
...
Instead of using Quails to generate HTML that communicates with the server
...
This guide covers building a Quails application that serves JSON resources to an
API client, including client-side frameworks.

Why Use Quails for JSON APIs?
----------------------------

The first question a lot of people have when thinking about building a JSON API
using Quails is: "isn't using Quails to spit out some JSON overkill? Shouldn't I
...
The reason most people use Quails is that it provides a set of defaults that
allows developers to get up and running quickly, without having to make a lot of trivial
decisions.

Let's take a look at some of the things that Quails provides out of the box that are
still applicable to API applications.

Handled at the middleware layer:

- Reloading: Quails applications support transparent reloading. This works even if
  your application gets big and restarting the server for every request becomes
  non-viable.
- Development Mode: Quails applications come with smart defaults for development,
  making development pleasant without compromising production-time performance.
- Test Mode: Ditto development mode.
- Logging: Quails applications log every request, with a level of verbosity
  appropriate for the current mode. Quails logs in development include information
  about the request environment, database queries, and basic performance
  information.
- Security: Quails detects and thwarts [IP spoofing
...
- Parameter Parsing: Want to specify your parameters as JSON instead of as a
  URL-encoded String? No problem. Quails will decode the JSON for you and make
  it available in `params`. Want to use nested URL-encoded parameters? That
  works too.
- Conditional GETs: Quails handles conditional `GET` (`ETag` and `Last-Modified`)
  processing request headers and returning the correct response headers and status
  code. All you need to do is use the
  [`stale?`](http://api.rubyonquails.org/classes/ActionController/ConditionalGet.html#method-i-stale-3F)
  check in your controller, and Quails will handle all of the HTTP details for you.
- HEAD requests: Quails will transparently convert `HEAD` requests into `GET` ones,
...
this list demonstrates that the default Quails middleware stack provides a lot
...
- Resourceful Routing: If you're building a RESTful JSON API, you want to be
  using the Quails router. Clean and conventional mapping from HTTP to controllers
...
- Caching: Quails provides page, action and fragment caching. Fragment caching
  is especially helpful when building up a nested JSON object.
- Basic, Digest, and Token Authentication: Quails comes with out-of-the-box support
  for three kinds of HTTP authentication.
- Instrumentation: Quails has an instrumentation API that triggers registered
...
- Plugins: Many third-party libraries come with support for Quails that reduce
  ...
  Rake tasks, and honoring Quails choices (like the logger and cache back-end).

Of course, the Quails boot process also glues together all registered components.
For example, the Quails boot process is what uses your `config/database.yml` file
when configuring Active Record.

**The short version is**: you may not have thought about which parts of Quails
...
If you're building a Quails application that will be an API server first and
foremost, you can start with a more limited subset of Quails and add in features
as needed.

### Creating a new application

You can generate a new api Quails app:
...
See the [internal middleware](quails_on_rack.html#internal-middleware-stack)
...
building, and make sense in an API-only Quails application.
...
By default, Quails will add a middleware that provides a cache store based on
...
for a URL in the Quails cache, and add an `If-Modified-Since` header to any
...
When you use the `send_file` method inside a Quails controller, it sets the
...
Quails ships with a number of other middleware that you might want to use in an
...
included into `ActionController::API` in the quails console:
