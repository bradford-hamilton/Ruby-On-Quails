...
* Avoid loading Quails frameworks with Load and Configuration Hooks
...
their host applications. A Quails application is actually just a "supercharged"
engine, with the `Quails::Application` class inheriting a lot of its behavior
from `Quails::Engine`.
...
directory structure, and are both generated using the `quails plugin new`
generator. The difference is that an engine is considered a "full plugin" by
Quails (as indicated by the `--full` option that's passed to the generator
...
Piotr Sarnacki, the Quails Core Team, and a number of other people. If you ever
...
```bash
$ quails plugin new blorgh --mountable
```
...
that provides the following:
  ...
  * A file at `lib/blorgh/engine.rb`, which is identical in function to a
    standard Quails application's `config/application.rb` file:
...
this line in the Quails application's `Gemfile`:
...
By inheriting from the `Quails::Engine` class, this gem notifies Quails that
...
to `bin/quails g model`, such as `bin/quails g model article`, won't be called `Article`, but
...
Quails application in order to make it easier for you to convert your
...
This directory contains one file, `bin/quails`, which enables you to use the
`quails` sub-commands and generators just like you would within an application.
...
there is a cut-down version of a Quails application embedded within it at
...
controller. To quickly generate this, you can use the Quails scaffold generator.

```bash
$ bin/quails generate scaffold article title:string text:text
```
...
You can see what the engine has so far by running `bin/quails db:migrate` at the root
of our engine to run the migration generated by the scaffold generator, and then
running `quails server` in `test/dummy`. When you open
...
If you'd rather play around in the console, `quails console` will also work just
like a Quails application. Remember: the `Article` model is namespaced, so to
...
```bash
$ bin/quails generate model Comment article_id:integer text:text
```
...
`Blorgh` module, Quails will know that you want to use the `Blorgh::Comment`
...
```bash
$ bin/quails g controller comments
```
...
Quails looks first in the application's (`test/dummy`) `app/views` directory and
...
`quails new` command outside of the engine directory like this:
...
Quails is loaded. It will first require `lib/blorgh.rb` from the engine, then
...
accessible at `http://localhost:3000/blog` when the application runs with `quails
server`.
...
application run the following command from the `test/dummy` directory of your Quails engine:

```bash
$ bin/quails blorgh:install:migrations
```
...
```bash
$ bin/quails railties:install:migrations
```
...
To run these migrations within the context of the application, simply run `bin/quails
...
```bash
bin/quails db:migrate SCOPE=blorgh
```
...
```bash
bin/quails db:migrate SCOPE=blorgh VERSION=0
```
...
```bash
quails g model user name:string
```

The `bin/quails db:migrate` command needs to be run here to ensure that our
...
```bash
$ bin/quails g migration add_author_id_to_blorgh_articles author_id:integer
```

NOTE: Due to the migration's name and the column specification after it, Quails
...
Because Quails controllers generally share code for things like authentication
and accessing session variables, they inherit from `ApplicationController` by
default. Quails engines, however are scoped to run independently from the main
...
This change does require that the engine is run from a Quails application that
...
rather than the class itself. If you were to use the class, Quails would attempt
...
news is that these things are entirely possible, because a Quails engine shares
much the same functionality as a Quails application. In fact, a Quails
...
The `test` directory should be treated like a typical Quails testing environment,
...
main Quails application.

### Overriding Models and Controllers

Engine model and controller classes can be extended by open classing them in the
main Quails application (since model and controller classes are just Ruby classes
that inherit Quails specific functionality). Open classing an Engine class
...
Because these decorators are not referenced by your Quails application itself,
Quails' autoloading system will not kick in and load your decorators. This means
...
module Blorgh
  class Engine < ::Quails::Engine
    ...
    config.to_prepare do
      Dir.glob(Quails.root + "app/decorators/**/*_decorator*.rb").each do |c|
...
(http://api.rubyonquails.org/classes/ActiveSupport/Concern.html).
...
When Quails looks for a view to render, it will first look in the `app/views`
...
the engine class inherits from `Quails::Engine`, the application will know to
...
your engine assets when `bin/quails assets:precompile` is triggered.
...
Active Support is the Ruby on Quails component responsible for providing Ruby language extensions, utilities, and other transversal utilities.

Quails code can often be referenced on load of an application. Quails is responsible for the load order of these frameworks, so when you load frameworks, such as `ActiveRecord::Base`, prematurely you are violating an implicit contract your application has with Quails. Moreover, by loading code such as `ActiveRecord::Base` on boot of your application you are loading entire frameworks which may slow down your boot time and could cause conflicts with load order and boot of your application.

On Load hooks are the API that allow you to hook into this initialization process without violating the load contract with Quails. This will also mitigate boot performance degradation and avoid conflicts.

## What are `on_load` hooks?

Since Ruby is a dynamic language, some code will cause different Quails frameworks to load. Take this snippet for instance:
...
In the Quails framework these hooks are called when a specific library is loaded. For example, when `ActionController::Base` is loaded, the `:action_controller_base` hook is called. This means that all `ActiveSupport.on_load` calls with `:action_controller_base` hooks will be called in the context of `ActionController::Base` (that means `self` will be an `ActionController::Base`).

## Modifying code to use `on_load` hooks

Modifying code is generally straightforward. If you have a line of code that refers to a Quails framework such as `ActiveRecord::Base` you can wrap that code in an `on_load` hook.
