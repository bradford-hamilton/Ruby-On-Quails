...
module Quails
  module Command
    class SecretsCommand < Quails::Command::Base # :nodoc:
      no_commands do
        def help
          say "Usage:\n  #{self.class.banner}"
          say ""
      ...
      def edit
        if ENV["EDITOR"].to_s.empty?
          ...
          say ""
          say %(EDITOR="mate --wait" bin/quails secrets:edit)
          say ""
      ...
      rescue Quails::Secrets::MissingKeyError => error
        say error.message
      rescue Errno::ENOENT => error
        raise unless error.message =~ /secrets\.yml\.enc/

        Quails::Secrets.read_template_for_editing do |tmp_path|
          system("#{ENV["EDITOR"]} #{tmp_path}")
      ...
      def show
        say Quails::Secrets.read
      end

      private
        def generator
          require_relative "../../generators"
          require_relative "../../generators/quails/encrypted_secrets/encrypted_secrets_generator"
