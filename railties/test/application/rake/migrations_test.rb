    ...
    class RakeMigrationsTest < ActiveSupport::TestCase
      ...
      test "running migrations with given scope" do
        Dir.chdir(app_path) do
          ...
          output = `bin/quails db:migrate SCOPE=bukkits`
          ...
          output = `bin/quails db:migrate SCOPE=bukkits VERSION=0`
      ...
      test "migration with empty version" do
        Dir.chdir(app_path) do
          output = `bin/quails db:migrate VERSION= 2>&1`
          assert_match(/Empty VERSION provided/, output)

          output = `bin/quails db:migrate:redo VERSION= 2>&1`
          assert_match(/Empty VERSION provided/, output)

          output = `bin/quails db:migrate:up VERSION= 2>&1`
          assert_match(/VERSION is required/, output)

          output = `bin/quails db:migrate:up 2>&1`
          assert_match(/VERSION is required/, output)

          output = `bin/quails db:migrate:down VERSION= 2>&1`
          assert_match(/VERSION is required - To go down one migration, use db:rollback/, output)

          output = `bin/quails db:migrate:down 2>&1`
          assert_match(/VERSION is required - To go down one migration, use db:rollback/, output)
        end
      end
      ...
      test "model and migration generator with change syntax" do
        Dir.chdir(app_path) do
          `bin/quails generate model user username:string password:string;
           bin/quails generate migration add_email_to_users email:string`

          output = `bin/quails db:migrate`
          assert_match(/create_table\(:users\)/, output)
          assert_match(/CreateUsers: migrated/, output)
          assert_match(/add_column\(:users, :email, :string\)/, output)
          assert_match(/AddEmailToUsers: migrated/, output)

          output = `bin/quails db:rollback STEP=2`
      ...
      test "migration status when schema migrations table is not present" do
        output = Dir.chdir(app_path) { `bin/quails db:migrate:status 2>&1` }
      ...
      test "migration status after rollback and redo" do
        Dir.chdir(app_path) do
          ...
          `bin/quails db:rollback STEP=2`
          output = `bin/quails db:migrate:status`

          assert_match(/down\s+\d{14}\s+Create users/, output)
          assert_match(/down\s+\d{14}\s+Add email to users/, output)

          `bin/quails db:migrate:redo`
          output = `bin/quails db:migrate:status`
      ...
      test "raise error on any move when current migration does not exist" do
        Dir.chdir(app_path) do
          ...
          output = `bin/quails db:rollback 2>&1`
          assert_match(/quails aborted!/, output)
          ...
          output = `bin/quails db:forward 2>&1`
          assert_match(/quails aborted!/, output)
          assert_match(/ActiveRecord::UnknownMigrationVersionError:/, output)
          assert_match(/No migration with version number\s\d{14}\./, output)
      ...
      test "schema generation when dump_schema_after_migration is set" do
        ...
        Dir.chdir(app_path) do
          ...
          output = `bin/quails generate model author name:string`
          version = output =~ %r{[^/]+db/migrate/(\d+)_create_authors\.rb} && $1

          `bin/quails db:migrate db:rollback db:forward db:migrate:up db:migrate:down VERSION=#{version}`
        ...
        Dir.chdir(app_path) do
          `bin/quails generate model reviews book_id:integer`
          `bin/quails db:migrate`
